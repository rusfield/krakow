...
<section class="bg-white py-20 sm:py-28">
	<div class="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
		<div class="mx-auto mb-12 max-w-xl text-center">
			<h2 class="text-3xl font-bold tracking-tight text-gray-900 sm:text-4xl">{t.rules.title}</h2>
			<p class="mt-4 text-lg text-gray-500">{t.rules.subtitle}</p>
		</div>

		<div class="mx-auto grid max-w-3xl grid-cols-1 gap-3 sm:grid-cols-2">
			{#each t.rules.items as rule}
				<div class="flex items-start gap-3 rounded-xl border border-gray-100 bg-gray-50 px-5 py-4">
					{#if rule.allowed}
						<span
							class="mt-0.5 flex size-6 shrink-0 items-center justify-center rounded-full bg-green-100 text-green-600"
						>
							<svg
					...
					{:else}
						<span
							class="mt-0.5 flex size-6 shrink-0 items-center justify-center rounded-full bg-red-100 text-red-500"
						>
							<svg
								xmlns="http://www.w3.org/2000/svg"
								viewBox="0 0 20 20"
					...
					{/if}
					<span class="text-sm leading-relaxed text-gray-700">{rule.text}</span>
				</div>
			{/each}
		</div>
	</div>
